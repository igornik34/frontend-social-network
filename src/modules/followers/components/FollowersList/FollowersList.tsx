...
import { useGetFollowersInfiniteQuery } from "../../services/api.ts";
...
export function FollowersList({userId, query}: Props) {
    const {
        data,
        fetchNextPage,
        hasNextPage,
        isFetching,
    } = useGetFollowersInfiniteQuery({userId, query});
    ...
    if (!isFetching && posts.length === 0) {
        return (
            <Center>
                <Text>У Вас нет подписчиков</Text>
            </Center>
        );
    }
            ...
            <Virtuoso
                ...
                itemContent={(index, follower) => (
                    <div style={{ padding: "8px 0" }}>
                        <FollowerCard key={follower.id} cardFor={'follower'} isFollowing={follower.isFollowing} follower={follower} />
                    </div>
                )}
                endReached={loadMore}
                components={{
